...
public class PdfService {
    ...
    public PdfService(TemplateEngine e,
                      @Value("$"+"{news.upload-dir:./data/uploads}") String dir) {
        engine = e;
        uploadDir = Paths.get(dir).toAbsolutePath().normalize();
    }

    public byte[] render(NewsArticle a, String brand, String tagline,
                         String location, String slogan) throws IOException {
        Context c = new Context();
        c.setVariable("article", a);
        c.setVariable("brand", brand);
        c.setVariable("tagline", tagline);
        c.setVariable("location", location);
        c.setVariable("slogan", slogan);
        ...
        if (a.getMainImage() != null && a.getMainImage().startsWith("/uploads/")) {
            String name = a.getMainImage().substring("/uploads/".length());
            ...
            if (p.startsWith(uploadDir) && Files.exists(p)) {
                ...
                html = html.replace(a.getMainImage(), data);
            }
        }
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            File regular = new File("/usr/share/fonts/truetype/noto/NotoSansDevanagari-Regular.ttf");
            File bold = new File("/usr/share/fonts/truetype/noto/NotoSansDevanagari-Bold.ttf");

            if (regular.exists()) b.useFont(regular, "NotoDevanagari");
            if (bold.exists()) b.useFont(bold, "NotoDevanagari");

            b.withHtmlContent(html, "http://localhost/");
            b.toStream(out);
            b.run();
